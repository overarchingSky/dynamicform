...
<el-select slot="reference" ref="select" :allow-create="allowInput" :filterable="allowInput" :placeholder="placeholder" :value="inputValue" @input="updateValue"  v-bind="$attrs" 
 :multiple='multiple'  v-on="$listeners" @focus="_ => visible = false">
    ...
    <el-input v-if="isInput" placeholder="请填写" @input="updateInputValue" :value="elValue"/>
	</div>
</template>
<script>
export default {
	name: "v-selectWithInput",
	props: {
		value: {
	...
	created() {
		...
		if (this.value) {
			...
			if (isExtra) {
				...
				this.inputValue = this.showExtraInput;
				this.isInput = true;
			} else {
